    ...
    >
      <section className="container-shell grid gap-8 pb-24 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
        ...
        <div className="relative">
          <OperationalMesh variant="compact" className="absolute inset-0 opacity-35" />
          <CaseOperationsMap />
        </div>
      </section>
      ...
      <section className="container-shell grid gap-4 pb-24 lg:grid-cols-4">
        {[
          ...
          <article key={label} className="rounded-[1.6rem] border border-white/10 p-5 surface-panel">
        ...
        <div className="rounded-[2rem] border border-white/10 p-6 surface-panel lg:p-8">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            {["42 km controlados", "128 ha georreferenciadas", "0.8 mm de tolerancia"].map((item) => (
              <div key={item} className="rounded-[1.5rem] border border-white/10 bg-black/20 p-5 text-slate-200">
                {item}
              </div>
            ))}
          </div>
